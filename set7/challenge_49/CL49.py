...
def frontend_send_message(sender, recipient, amount):
    ...
    iv = util.randbytes(16)           #随机生成IV 
    print ('iv:',iv)
    last_sent_message = message + iv + CBC_MAC(key, iv, message)     #用message 和IV 生成MAC
...
def attacker_send_forged_message(sender, recipient, amount):
    ...
    fake_sender = (b'M' * min(len(sender), 11)) + sender[11:]         #随便生成 用户名  'MMM' 只要长度与 'TOM'一致 
    print (fake_sender)
    # Assume this message fails or otherwise has no effect.
    frontend_send_message(fake_sender, recipient, amount)          #本地计算MAC   
    m = attacker_peek_last_sent_message()
    print ('dddd:',m)
    message, iv, mac = m[:-32], m[-32:-16], m[-16:]
    forged_message = b'from=' + sender + message[len(sender)+5:]    #伪造message 改用户名'MMM'为 TOM 发给jinlaoban
    print ('forged_message:::',forged_message)
    forged_iv = strxor(iv, strxor(message[:16], forged_message[:16]))         #  iv 异或message异或forge_message
    print ('forged_iv:::',forged_iv)
...
if __name__ == '__main__':
    attacker_send_forged_message(b'Tom', b'jinlaoban', b'1000000')        #想伪造  TOM 给jinlaoban  10000000
